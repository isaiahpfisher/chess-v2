...
export class Board {
  ...
  isValidMove(turn, originId, destId) {
    ...
    if (endPiece.type == KING) {
      validMove = false;
      Game.invalidMessage = `You cannot capture a king. Please try again.`;
    }
  ...
  evaluateBoard(color) {
    ...
    // LOOK FOR CHECKMATE
    let isMate = this.isMate(enemyColor);
    let isCheck = this.findKing(enemyColor).isInCheck(this.grid);

    // Evaluate the score based on checkmate
    if (isCheck && isMate) {
      score = 2000;
    }
    ...
    score +=
      2 * (enemyKing.isInCheck(this.grid) - currentKing.isInCheck(this.grid));
  ...
  /**
   * Generates an array of valid moves for a given color.
   *
   * @param {string} color - The color of the pieces.
   * @returns {Array} - An array of valid moves, each represented by an object with the properties 'originId' and 'destId'.
   */
